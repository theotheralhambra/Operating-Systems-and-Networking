...
int main( int argc, char *argv[] ){ 															// argv[0] = exe, argv[1] = hostname, argv[2] = port
	...
	while( 1 ) {
		...
		//parse_command(comm_buffer, command, filename, dataport);
		switch(parse_command(comm_buffer, command, filename, dataport)) {
			case 1:																											// list
				list_files(file_buffer);
				dataFD = setup_data_socket(clientName, atoi(dataport));
				send_data(dataFD, file_buffer);
				break;
			case 2:																											// get
				dataFD = setup_data_socket(clientName, atoi(dataport));
				read_file(filename, file_buffer, FILE_BUF_SIZE);
				send_data(dataFD, file_buffer);
				printf("ftserver: transfer complete\n");
				break;
			case 3:
				printf("ftserver: ERROR invalid file name specified.\n");	// get with bad filename
				dataFD = setup_data_socket(clientName, atoi(dataport));
				send_data(dataFD, "ERROR");
				if (send(controlConnectionFD, "ftserver: ERROR invalid file name specified.", 44, 0) < 0)
					error("ftserver: ERROR writing to socket.");
				break;
			default:																										// invalid call
				...
				dataFD = setup_data_socket(clientName, atoi(dataport));
				send_data(dataFD, "ERROR");
				if (send(controlConnectionFD, "ftserver: ERROR invalid or malformed command specified.", 55, 0) < 0)
					error("ftserver: ERROR writing to socket.");
